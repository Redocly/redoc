...
export function highlight(source: string | number | boolean, lang: string = DEFAULT_LANG): string {
  ...
  const grammar =
    Prism.languages[normalizedLang] || Prism.languages[mappedLang] || Prism.languages[DEFAULT_LANG];

  const languageForPrism = Prism.languages[mappedLang]
    ? mappedLang
    : Prism.languages[normalizedLang]
      ? normalizedLang
      : DEFAULT_LANG;

  return Prism.highlight(source.toString(), grammar, languageForPrism);
}
